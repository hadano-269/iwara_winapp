...
class HomeTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.sort_combo.addItems(['热门 (trending)', '最新 (date)', '播放量 (views)', '点赞数 (likes)', '人气 (popularity)'])
        self.sort_combo.currentIndexChanged.connect(self.on_sort_changed)
        toolbar.addWidget(self.sort_combo)

        self.rating_combo = QComboBox()
        self.rating_combo.addItems(['全部 (all)', '普通 (general)', '成人 (ecchi)'])
        self.rating_combo.currentIndexChanged.connect(self.on_sort_changed)
        ...
        self.loading_label.setStyleSheet('font-size: 12px; color: #999999; background: transparent;')
        layout.addWidget(self.loading_label)

    def get_sort_value(self):
        return ['trending', 'date', 'views', 'likes', 'popularity'][self.sort_combo.currentIndex()]

    def get_rating_value(self):
        return ['all', 'general', 'ecchi'][self.rating_combo.currentIndex()]
    ...
    def load_videos(self):
        self.loading_label.setText('正在加载视频列表...')
        self.clear_grid()
        sort = self.get_sort_value()
        rating = self.get_rating_value()
        if self.worker and self.worker.isRunning():
            self.worker.quit()
        self.worker = ApiWorker(self.api_client, 'get_videos', sort=sort, rating=rating, page=self.current_page, limit=32)
    ...
    def on_videos_loaded(self, result):
        ...
        try:
            ...
            self.loading_label.setText(f'解析失败: {str(e)}')

    def on_load_error(self, err):
        self.loading_label.setText(f'加载失败: {err}')
